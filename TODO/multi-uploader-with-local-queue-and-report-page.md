...
- [ ] Extend the state machine in `App.svelte` so `scan` can hold a queue of files (pending, scanning, done, failed) instead of one path. Keep the single-file path working.
- [ ] Update `DropZone.svelte` to accept multi-file drop and multi-select in the file dialog. Filter to `.jar`, reject the rest with a single grouped message.
- [ ] Build a `QueuePanel.svelte` component listing each file with status icon, name, size, severity counts, and an inline progress bar. Per-row actions: cancel (pending), retry (failed), open report (done).
...
- [ ] Add a `ReportPage.svelte` aggregate view: total files, totals per severity across the batch, list of files sorted by worst severity, click to drill into the per-file `SignatureList`.
...
- [ ] Update `RemoteStatus` to also pause the queue if the API goes offline mid-batch, and resume on the next successful health check.
